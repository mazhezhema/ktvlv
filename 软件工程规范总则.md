...
**典型案例**（2025-12-26）：
- **问题**：主界面卡住未响应
- **根因**：`CacheService::initialize()` 在主线程调用 `std::filesystem::create_directories()`，在网络驱动器、权限问题等场景下阻塞
- **修复**：将 `CacheService::initialize()` 移到后台线程执行
- **详细分析**：参见 `问题根因分析-主界面卡住未响应.md`
...
| **文件系统操作** | `std::filesystem::create_directories()`, `std::filesystem::exists()`, `std::filesystem::remove()` | 0.1-10秒 | 🟠 高 |
...
**问题根因**（2025-12-26 发现）：
- `std::filesystem::create_directories()` 在主线程执行时可能阻塞
- **阻塞场景**：
  - 网络驱动器（映射的网络盘）
  - 权限问题（需要用户确认）
  - 磁盘IO慢（机械硬盘、USB设备）
  - 防病毒软件扫描
- **症状**：UI 卡住未响应，Windows 显示"未响应"
- **修复**：所有文件系统操作必须在后台线程执行

**典型案例**：
```cpp
// ❌ 错误：在主线程初始化 CacheService
CacheService::getInstance().initialize("cache");  // 内部调用 create_directories()
...
- [ ] **主线程中是否有文件系统操作？**
  - 搜索：`std::filesystem::create_directories(`, `std::filesystem::exists(`, `std::filesystem::remove(`
  - **所有文件系统操作都必须在后台线程执行**（即使操作很快）
...
- [ ] **初始化顺序是否正确？**
  ...
  - 主循环是否在服务初始化之后立即开始？
  - **服务初始化是否在主线程执行？**
    - 检查：`CacheService::initialize()` 是否在主线程调用？
    - 检查：`HttpService::initialize()` 是否在主线程调用？（可以，因为只是设置配置）
    - 检查：所有文件系统操作是否在后台线程执行？
...
> **本章节记录从实际项目中总结的软件工程经验和最佳实践，这些经验来自真实问题的解决过程。**

### 7.1 文件系统操作的隐藏陷阱（2025-12-26）

#### 问题背景

在主界面卡住未响应的问题中，我们发现了一个容易被忽视的陷阱：**文件系统操作在主线程执行**。

#### 核心教训

**经验 1：文件系统操作的阻塞时间不可预测**

```cpp
// ❌ 错误认知：认为文件系统操作很快，可以在主线程执行
std::filesystem::create_directories("cache");  // 看起来很快，但可能阻塞！

// ✅ 正确认知：文件系统操作必须异步，因为阻塞时间不可预测
TaskService::runAsync([]() {
    std::filesystem::create_directories("cache");  // 在后台线程执行
});
```

**阻塞场景（实际遇到或可能遇到）**：
- 网络驱动器（映射的网络盘）- 可能阻塞 1-10 秒
- 权限问题（需要用户确认）- 可能阻塞 5-30 秒
- 磁盘IO慢（机械硬盘、USB设备）- 可能阻塞 0.1-5 秒
- 防病毒软件扫描 - 可能阻塞 0.5-10 秒
- 文件系统锁定 - 可能阻塞 0.1-5 秒

**关键洞察**：
> **即使操作通常很快（<100ms），也不应该在主线程执行，因为阻塞时间不可预测。**
...
1. **检查控制台输出**
   - 如果看到初始化消息，但 UI 不响应 → 主线程被阻塞
   - 如果看不到主循环消息 → 主循环没有启动

2. **检查初始化顺序**
   - 搜索：`Service::getInstance().initialize(`
   - 确认：是否有文件系统操作？是否有网络请求？
...
4. **使用调试输出**
   ```cpp
   printf("[MAIN] Entering main loop NOW...\n");
   printf("[Background] Initializing CacheService...\n");
   printf("[UI Thread] Home tab displayed...\n");
   ```
...
TaskService::runAsync([]() {
    try {
        ...
        // 错误处理：在 UI 线程显示错误
        TaskService::runOnUIThread([e]() {
            showError("初始化失败: " + std::string(e.what()));
        });
    }
});
```
...
> **文件系统操作必须异步，即使操作通常很快（<100ms），因为阻塞时间不可预测**

> **UI 必须在耗时操作之前显示，主循环必须在所有耗时操作之前启动**
...
- [问题根因分析-主界面卡住未响应.md](./问题根因分析-主界面卡住未响应.md)
