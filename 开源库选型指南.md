...
#### 方案2：moodycamel::ReaderWriterQueue（如果只需要SPSC）⭐ **推荐**
...
- HTTP客户端 → 使用 **libcurl**
- 配置文件解析器 → 使用 **inih**
- 消息队列 → 使用 **tinySPSCQueue**（SPSC，需验证）或 **moodycamel::ReaderWriterQueue**（SPSC）或 **moodycamel::ConcurrentQueue**（MPSC）
- 复杂的数据结构 → 使用标准库容器
...
- **cJSON** - JSON解析 ⭐ 必需
- **libcurl** - HTTP REST API客户端 ⭐ 必需
- **消息队列库** ⭐ 必需（根据需求选择：tinySPSCQueue、moodycamel::ReaderWriterQueue或moodycamel::ConcurrentQueue）
- **inih** - 配置文件解析（可选）
- **plog** - 日志库（可选，推荐）
- **标准库** - 数据结构、字符串、文件操作
...
5. **消息队列库** ⭐ 必需（线程间通信，解耦合）
   - **tinySPSCQueue**（如果只需要SPSC，需验证）⭐ 推荐
   - **moodycamel::ReaderWriterQueue**（如果只需要SPSC）⭐ 推荐
