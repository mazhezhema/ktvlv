...
```
          ...
          └─────▲────────┘
                │
   下载完成      │SDK回调                   UI操作
  m3u8/ts就绪    │播放事件                  点击/输入
        │        │                          │
        │        │                          │
        ▼        ▼                          ▼
[Download Thread]  [tplayer thread]   [UI Thread]
        │                │                  │
        └─────send──────┴───send───────────┘
                         │
                         ▼
...
UI主线程同步处理：
  ...
  - SQLite访问（同步）
  - UI刷新（LVGL控件更新）
  - 播放器命令（直接调用PlayerService）
```

---

## 🚦 MVP简化说明（仅 UiEventQueue）

> 本规范原版包含 `PlayerCmdQueue + UiEventQueue` 双队列模型。MVP阶段已简化为：  
> - 仅保留 `UiEventQueue`（下载线程/播放器回调 → UI 主线程）  
> - UI → 播放器命令通过 `PlayerService` 直接封装调用（不引入命令队列）  
> - 普通网络/SQLite 在主线程同步执行  
> 若后续进入量产阶段，再恢复 `PlayerCmdQueue` 相关章节。
...
| **下载事件** | m3u8/ts下载完成/进度 | 更新业务逻辑→UI更新 | **UiEventQueue** | UI线程 |
| **播放器事件** | prepared/playing/ended | 驱动UI状态机 | **UiEventQueue** | UI线程 |
| **播放器控制命令** | play/pause/seek | 直接调用PlayerService | ❌ **不进队列**（主线程同步） | UI线程 |

### 关键规则（MVP简化版）

✔️ **下载线程、播放器回调 → 统一通过 UiEventQueue 回流UI**  
✔️ **UI → 播放器 → 直接调用PlayerService（主线程同步）**  
✔️ **UI控件本地行为 = 不用队列，直接处理**  
✔️ **普通网络请求/SQLite = 主线程同步执行**
...
- [事件模型MVP实现指南（可落地版）.md](../guides/事件模型MVP实现指南（可落地版）.md) ⭐⭐⭐ **必读** - MVP 级完整实现示例
- [应用层命名规范（架构约束版）.md](../guides/应用层命名规范（架构约束版）.md) ⭐⭐⭐ **必读** - 命名规范
- [线程架构基线（最终版）.md](../线程架构基线（最终版）.md) ⭐⭐⭐ **必读** - 线程架构
- [消息队列完整指南.md](../消息队列完整指南.md) ⭐⭐⭐ **必读** - 消息队列实现
