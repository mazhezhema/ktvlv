# 应用启动阶段划分与退出原则

> **文档版本**：v1.0  
> **最后更新**：2025-12-30  
> **状态**：✅ 核心文档（架构设计）  
> **适用平台**：F133 / Tina Linux  
> **目标**：明确应用启动阶段划分和统一退出原则，确保问题可定位、退出可控

---

## 🎯 核心原则（一句话）

> **启动分4个明确阶段，退出必须与启动顺序相反，只有 AppRuntime 有权触发退出。**

---

## 📋 目录

1. [启动阶段划分](#一-启动阶段划分)
2. [退出原则](#二-退出原则)
3. [阶段诊断](#三-阶段诊断)
4. [实现示例](#四-实现示例)

---

## 一、启动阶段划分

### 阶段语义

应用启动分为 **4 个明确阶段**，每个阶段有明确的职责和完成标志。

### Stage 1：Platform Ready（平台就绪）

**职责**：确保底层平台资源可用

**检查项**：
- ✅ 文件系统挂载完成
- ✅ 网络接口可用（`/sys/class/net/eth0` 或 `/sys/class/net/wlan0`）
- ✅ 显示设备可用（`/dev/fb0` 可读写）
- ✅ 输入设备可用（`/dev/input/eventX` 可读写）

**完成标志**：所有平台资源检查通过

**失败处理**：记录错误日志，退出应用（返回码 1）

---

### Stage 2：Core Runtime（核心运行时）

**职责**：初始化核心运行时组件

**检查项**：
- ✅ AppRuntime 初始化完成
- ✅ EventBus 启动完成
- ✅ 日志系统初始化完成（syslog 可用）
- ✅ 配置系统加载完成

**完成标志**：所有核心运行时组件初始化成功

**失败处理**：记录错误日志，退出应用（返回码 2）

---

### Stage 3：Service Init（服务初始化）

**职责**：初始化业务服务

**检查项**：
- ✅ NetworkWorker 启动完成
- ✅ PlayerAdapter 启动完成（TPlayer 初始化）
- ✅ StorageService 初始化完成（SQLite 可用）
- ✅ HistoryService 初始化完成

**完成标志**：所有业务服务初始化成功

**失败处理**：记录错误日志，尝试回退到 Stage 2，如果失败则退出应用（返回码 3）

---

### Stage 4：UI Ready（UI就绪）

**职责**：初始化 UI 系统并显示首屏

**检查项**：
- ✅ LVGL 初始化完成
- ✅ 显示驱动初始化完成
- ✅ 输入驱动初始化完成
- ✅ 首屏渲染完成

**完成标志**：UI 系统可用，首屏显示正常

**失败处理**：记录错误日志，尝试回退到 Stage 3，如果失败则退出应用（返回码 4）

---

## 二、退出原则

### 核心原则

1. **UI 不直接 exit**
   - UI 层只能发送退出事件
   - 禁止直接调用 `exit()` 或 `abort()`

2. **模块不直接 exit**
   - 业务模块只能通知 AppRuntime
   - 禁止直接调用 `exit()` 或 `abort()`

3. **只有 AppRuntime 有权触发退出**
   - 所有退出请求必须通过 AppRuntime
   - AppRuntime 统一处理退出逻辑

4. **退出顺序必须与启动顺序相反**
   - Stage 4 → Stage 3 → Stage 2 → Stage 1
   - 确保资源按依赖关系正确释放

### 退出流程

```
退出请求
    ↓
AppRuntime::requestExit()
    ↓
Stage 4 清理（UI 系统）
    ↓
Stage 3 清理（业务服务）
    ↓
Stage 2 清理（核心运行时）
    ↓
Stage 1 清理（平台资源）
    ↓
exit(0)
```

---

## 三、阶段诊断

### 问题定位

**出了问题，能一眼判断"死在哪个阶段"**

| 阶段 | 返回码 | 典型问题 | 诊断方法 |
|------|--------|---------|---------|
| **Stage 1** | 1 | 文件系统未挂载、网络接口不可用 | 检查 `/sys/class/net/` |
| **Stage 2** | 2 | EventBus 启动失败、日志系统不可用 | 检查 syslog |
| **Stage 3** | 3 | TPlayer 初始化失败、SQLite 不可用 | 检查服务日志 |
| **Stage 4** | 4 | LVGL 初始化失败、显示驱动不可用 | 检查 framebuffer |

### 日志记录

每个阶段开始和结束时记录日志：

```cpp
syslog(LOG_INFO, "[ktv][startup] Stage 1: Platform Ready - Starting...");
// ... 初始化逻辑 ...
syslog(LOG_INFO, "[ktv][startup] Stage 1: Platform Ready - Completed");

syslog(LOG_INFO, "[ktv][startup] Stage 2: Core Runtime - Starting...");
// ... 初始化逻辑 ...
syslog(LOG_INFO, "[ktv][startup] Stage 2: Core Runtime - Completed");
```

---

## 四、实现示例

### 启动流程

```cpp
class AppRuntime {
public:
    enum class StartupStage {
        PLATFORM_READY = 1,
        CORE_RUNTIME = 2,
        SERVICE_INIT = 3,
        UI_READY = 4
    };

    int startup() {
        // Stage 1: Platform Ready
        syslog(LOG_INFO, "[ktv][startup] Stage 1: Platform Ready - Starting...");
        if (!initPlatform()) {
            syslog(LOG_ERR, "[ktv][startup] Stage 1 failed");
            return 1;
        }
        syslog(LOG_INFO, "[ktv][startup] Stage 1: Platform Ready - Completed");

        // Stage 2: Core Runtime
        syslog(LOG_INFO, "[ktv][startup] Stage 2: Core Runtime - Starting...");
        if (!initCoreRuntime()) {
            syslog(LOG_ERR, "[ktv][startup] Stage 2 failed");
            cleanupPlatform();
            return 2;
        }
        syslog(LOG_INFO, "[ktv][startup] Stage 2: Core Runtime - Completed");

        // Stage 3: Service Init
        syslog(LOG_INFO, "[ktv][startup] Stage 3: Service Init - Starting...");
        if (!initServices()) {
            syslog(LOG_ERR, "[ktv][startup] Stage 3 failed");
            cleanupCoreRuntime();
            cleanupPlatform();
            return 3;
        }
        syslog(LOG_INFO, "[ktv][startup] Stage 3: Service Init - Completed");

        // Stage 4: UI Ready
        syslog(LOG_INFO, "[ktv][startup] Stage 4: UI Ready - Starting...");
        if (!initUI()) {
            syslog(LOG_ERR, "[ktv][startup] Stage 4 failed");
            cleanupServices();
            cleanupCoreRuntime();
            cleanupPlatform();
            return 4;
        }
        syslog(LOG_INFO, "[ktv][startup] Stage 4: UI Ready - Completed");

        syslog(LOG_INFO, "[ktv][startup] All stages completed successfully");
        return 0;
    }

    void requestExit() {
        syslog(LOG_INFO, "[ktv][shutdown] Exit requested, starting cleanup...");

        // Stage 4 清理（UI 系统）
        cleanupUI();

        // Stage 3 清理（业务服务）
        cleanupServices();

        // Stage 2 清理（核心运行时）
        cleanupCoreRuntime();

        // Stage 1 清理（平台资源）
        cleanupPlatform();

        syslog(LOG_INFO, "[ktv][shutdown] Cleanup completed, exiting...");
        exit(0);
    }

private:
    bool initPlatform() {
        // 检查文件系统
        if (access("/tmp", F_OK) != 0) {
            syslog(LOG_ERR, "[ktv][platform] Filesystem not ready");
            return false;
        }

        // 检查网络接口
        if (access("/sys/class/net/eth0", F_OK) != 0 &&
            access("/sys/class/net/wlan0", F_OK) != 0) {
            syslog(LOG_ERR, "[ktv][platform] Network interface not available");
            return false;
        }

        // 检查显示设备
        if (access("/dev/fb0", R_OK | W_OK) != 0) {
            syslog(LOG_ERR, "[ktv][platform] Display device not available");
            return false;
        }

        return true;
    }

    bool initCoreRuntime() {
        // 初始化 AppRuntime
        // 初始化 EventBus
        EventBus::getInstance().start();

        // 初始化日志系统（syslog 已由系统提供）
        openlog("ktvlv", LOG_PID | LOG_CONS, LOG_USER);

        // 加载配置
        Config::instance().load();

        return true;
    }

    bool initServices() {
        // 启动 NetworkWorker
        NetworkWorker::instance().start();

        // 启动 PlayerAdapter
        PlayerAdapter::instance().start();

        // 初始化 StorageService
        StorageService::instance().init();

        // 初始化 HistoryService
        HistoryService::instance().init();

        return true;
    }

    bool initUI() {
        // 初始化 LVGL
        lv_init();

        // 初始化显示驱动
        if (!initDisplayDriver()) {
            return false;
        }

        // 初始化输入驱动
        if (!initInputDriver()) {
            return false;
        }

        // 渲染首屏
        renderFirstScreen();

        return true;
    }

    void cleanupUI() {
        syslog(LOG_INFO, "[ktv][shutdown] Cleaning up UI...");
        // UI 清理逻辑
    }

    void cleanupServices() {
        syslog(LOG_INFO, "[ktv][shutdown] Cleaning up services...");
        PlayerAdapter::instance().stop();
        NetworkWorker::instance().stop();
    }

    void cleanupCoreRuntime() {
        syslog(LOG_INFO, "[ktv][shutdown] Cleaning up core runtime...");
        EventBus::getInstance().stop();
    }

    void cleanupPlatform() {
        syslog(LOG_INFO, "[ktv][shutdown] Cleaning up platform...");
        closelog();
    }
};
```

### 使用示例

```cpp
int main() {
    int ret = AppRuntime::instance().startup();
    if (ret != 0) {
        syslog(LOG_ERR, "[ktv] Startup failed at stage %d", ret);
        return ret;
    }

    // 主循环
    while (!AppRuntime::instance().shouldQuit()) {
        lv_timer_handler();
        usleep(5000);
    }

    // 退出（由 AppRuntime 统一处理）
    AppRuntime::instance().requestExit();
    return 0;
}
```

---

## 五、关键规则（必须遵守）

### 🚫 禁止事项

1. **禁止 UI 直接 exit**
   - ❌ 禁止在 UI 回调中调用 `exit()`
   - ✅ 必须发送退出事件到 AppRuntime

2. **禁止模块直接 exit**
   - ❌ 禁止在业务模块中调用 `exit()`
   - ✅ 必须通知 AppRuntime

3. **禁止跳过阶段**
   - ❌ 禁止跳过某个启动阶段
   - ✅ 必须按顺序执行所有阶段

4. **禁止退出顺序错误**
   - ❌ 禁止不按相反顺序退出
   - ✅ 必须按 Stage 4 → 3 → 2 → 1 顺序退出

---

## 📚 相关文档

- [AppRuntime线程生命周期总控设计.md](./AppRuntime线程生命周期总控设计.md) ⭐⭐⭐ **必读**
- [异常态处理总则.md](./异常态处理总则.md) ⭐⭐⭐ **必读**
- [KTV_App稳定性与自愈设计说明.md](../sdk/KTV_App稳定性与自愈设计说明.md)

---

**最后更新**: 2025-12-30  
**维护者**: 项目团队  
**状态**: ✅ 核心文档（架构设计）


