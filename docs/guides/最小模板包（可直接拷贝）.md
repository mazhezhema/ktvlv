...
所有模板基于 **C++** 实现，符合项目架构规范。

---

## 1️⃣ Event 模板

### event_types.h（PM 定义，工程师只读）
...
## 2️⃣ EventQueue 模板（框架层，工程师只使用）

```cpp
// src/events/event_queue.h（框架层提供）
class EventQueue {
public:
...
## 3️⃣ EventDispatcher 模板（框架层，PM A）

```cpp
// src/events/event_dispatcher.h（框架层提供）
...
private:
    void dispatchLoop();  // EventDispatcher 线程循环
    void dispatch(const AppEvent& ev);  // switch 路由
};
...
void CategoryController::handleCategoryRequest(int categoryId) {
    // 内部处理：发送事件到 EventQueue
    AppEvent ev;
    ev.type = EventType::EVENT_CATEGORY_CLICK;
    ev.arg1 = categoryId;
    EventQueue::instance().enqueue(ev);
    
    // EventDispatcher 会路由到 CategoryService::onClick(categoryId)
}

void CategoryController::onSvcCategoryDataReady(int categoryId, void* data) {
    // Service 回调：处理网络返回的数据
...
## 6️⃣ NetworkService 模板（PM A接口，工程师B实现）

```cpp
// src/services/network_service.h（PM A接口）
#pragma once
#include "events/event_queue.h"
#include "events/event_types.h"
#include <string>
...
## 7️⃣ EventDispatcher 路由模板（PM A）

```cpp
// src/events/event_dispatcher.cpp（框架层实现）
...
## 8️⃣ Page 模板（Singleton，show/hide）
...
// 5. NetworkService：网络请求，结果通过 Event 返回
void NetworkService::fetchCategory(int categoryId) {
    // libcurl 请求...
    // 成功后发送 Event
    AppEvent ev;
...
// 6. EventDispatcher：路由到 Controller
void EventDispatcher::dispatch(const AppEvent& ev) {
    switch (ev.type) {
        case EventType::EVENT_CATEGORY_DATA_READY:
            CategoryController::instance().onSvcCategoryDataReady(ev.arg1, ev.data);
...
- [NetworkService与libcurl实现指南（MVP可落地版）.md](./NetworkService与libcurl实现指南（MVP可落地版）.md) ⭐⭐⭐ **必读**
