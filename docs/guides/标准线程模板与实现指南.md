# 标准线程模板与实现指南（F133 / Tina Linux）

> **文档版本**：v1.1  
> **最后更新**：2025-12-30  
> **状态**：✅ 参考文档（已合并到编码规范）  
> **适用平台**：F133 / Tina Linux  
> **目标**：为工程师提供可直接使用的标准线程模板

> **⚠️ 重要提示**：本文档已合并到 [KTV_App线程Singleton编码规范（最终版）.md](./KTV_App线程Singleton编码规范（最终版）.md)，建议直接参考编码规范文档。本文档保留作为历史参考。

---

## 🎯 核心原则（一句话）

> **线程尽量常驻，任务按需唤醒/休眠。线程是常驻资源，不是业务对象。**

> **📌 推荐阅读**：[KTV_App线程Singleton编码规范（最终版）.md](./KTV_App线程Singleton编码规范（最终版）.md) ⭐⭐⭐ **必读（铁律级）** - 包含完整的模板代码、避坑指南和工程铁律

---

## 📋 目录

1. [标准线程模板](#一-标准线程模板)
2. [三类常驻线程实现](#二-三类常驻线程实现)
3. [禁止的线程使用方式](#三-禁止的线程使用方式)
4. [线程生命周期管理](#四-线程生命周期管理)

---

## 一、标准线程模板

### 1.1 工作线程模板（Network/Player/Cache Worker）

**适用场景**：Network Worker、Player Worker、Cache Worker 等需要处理任务的常驻线程

```cpp
#include <thread>
#include <queue>
#include <mutex>
#include <condition_variable>
#include <atomic>
#include <syslog.h>

/**
 * @brief 标准工作线程模板
 * 
 * 特点：
 * - 线程常驻（App 启动时创建，App 退出时销毁）
 * - 任务按需唤醒/休眠（阻塞在 condition_variable）
 * - 不使用 busy-loop（while(1)+sleep）
 */
class StandardWorkerThread {
public:
    struct Task {
        int type;
        void* data;
        // ... 任务数据
    };

    StandardWorkerThread() : running_(false) {}
    
    ~StandardWorkerThread() {
        stop();
    }

    /**
     * @brief 启动线程
     */
    void start() {
        if (running_.load()) {
            return;  // 已经启动
        }
        
        running_.store(true);
        thread_ = std::thread(&StandardWorkerThread::workerLoop, this);
        syslog(LOG_INFO, "[ktv][thread] Worker thread started");
    }

    /**
     * @brief 停止线程
     */
    void stop() {
        if (!running_.load()) {
            return;  // 已经停止
        }
        
        running_.store(false);
        condition_.notify_all();  // 唤醒等待的线程
        
        if (thread_.joinable()) {
            thread_.join();  // 等待线程退出
        }
        
        syslog(LOG_INFO, "[ktv][thread] Worker thread stopped");
    }

    /**
     * @brief 添加任务
     */
    void enqueue(const Task& task) {
        {
            std::lock_guard<std::mutex> lock(mutex_);
            queue_.push(task);
        }
        condition_.notify_one();  // 唤醒一个等待的线程
    }

private:
    /**
     * @brief 工作循环（核心实现）
     * 
     * 关键点：
     * 1. 阻塞在 condition_variable（不 busy-loop）
     * 2. 检查 running_ 标志
     * 3. 处理任务
     * 4. 循环继续阻塞等待
     */
    void workerLoop() {
        syslog(LOG_INFO, "[ktv][thread] Worker thread loop started");
        
        while (running_.load()) {
            Task task;
            
            // 阻塞等待任务（关键：不 busy-loop）
            {
                std::unique_lock<std::mutex> lock(mutex_);
                
                // 等待条件：队列不为空 或 running_ 为 false
                condition_.wait(lock, [this]() {
                    return !queue_.empty() || !running_.load();
                });
                
                // 检查是否需要退出
                if (!running_.load()) {
                    break;
                }
                
                // 队列为空（异常情况，应该不会发生）
                if (queue_.empty()) {
                    continue;
                }
                
                // 取出任务
                task = queue_.front();
                queue_.pop();
            }
            
            // 处理任务（在锁外执行，避免长时间持锁）
            processTask(task);
        }
        
        syslog(LOG_INFO, "[ktv][thread] Worker thread loop exited");
    }

    /**
     * @brief 处理任务（子类实现）
     */
    virtual void processTask(const Task& task) {
        // 子类实现具体任务处理逻辑
        syslog(LOG_DEBUG, "[ktv][thread] Processing task type=%d", task.type);
    }

protected:
    std::thread thread_;
    std::atomic<bool> running_;
    std::queue<Task> queue_;
    std::mutex mutex_;
    std::condition_variable condition_;
};
```

### 1.2 使用示例：Network Worker

```cpp
class NetworkWorker : public StandardWorkerThread {
public:
    static NetworkWorker& instance() {
        static NetworkWorker inst;
        return inst;
    }

protected:
    void processTask(const Task& task) override {
        switch (task.type) {
            case TASK_HTTP_REQUEST:
                handleHttpRequest(task);
                break;
            case TASK_WEBSOCKET_SEND:
                handleWebSocketSend(task);
                break;
            // ...
        }
    }

private:
    void handleHttpRequest(const Task& task) {
        // HTTP 请求处理逻辑
        // ...
    }
};

// 使用方式
void initNetworkWorker() {
    NetworkWorker::instance().start();
}

void sendHttpRequest(const std::string& url) {
    StandardWorkerThread::Task task;
    task.type = TASK_HTTP_REQUEST;
    task.data = /* ... */;
    NetworkWorker::instance().enqueue(task);
}

void cleanupNetworkWorker() {
    NetworkWorker::instance().stop();
}
```

---

## 二、三类常驻线程实现

### 2.1 核心常驻线程（UI 主线程）

**特点**：App 生命周期，不阻塞（轮询模式）

```cpp
// UI 主线程（主循环）
int main() {
    // 初始化 LVGL
    lv_init();
    
    // 初始化显示和输入
    init_display();
    init_input();
    
    // 主循环（轮询模式，不阻塞）
    while (g_app_running) {
        lv_timer_handler();  // LVGL 定时器处理
        usleep(5000);        // 5ms，避免 CPU 占用过高
    }
    
    return 0;
}
```

### 2.2 工作线程池（Network/Player Worker）

**特点**：线程常驻，任务按需唤醒/休眠（使用上面的模板）

```cpp
// Network Worker 实现
class NetworkWorker : public StandardWorkerThread {
    // 使用标准模板
};

// Player Worker 实现
class PlayerWorker : public StandardWorkerThread {
    // 使用标准模板
};
```

### 2.3 低频守护线程（LogUpload/UpgradeChecker）

**特点**：周期唤醒，使用 sleep/timerfd

```cpp
/**
 * @brief 日志上传线程（低频守护线程）
 */
class LogUploadThread {
public:
    static LogUploadThread& instance() {
        static LogUploadThread inst;
        return inst;
    }

    void start() {
        running_.store(true);
        thread_ = std::thread(&LogUploadThread::uploadLoop, this);
    }

    void stop() {
        running_.store(false);
        if (thread_.joinable()) {
            thread_.join();
        }
    }

private:
    void uploadLoop() {
        while (running_.load()) {
            // 执行上传任务
            if (hasPendingLogs()) {
                uploadLogs();
            }
            
            // 阻塞等待（周期唤醒）
            // 方式1：使用 sleep（简单）
            sleep(60);  // 60 秒唤醒一次
            
            // 方式2：使用 timerfd（更精确，可选）
            // waitForTimer(60);
        }
    }

    void uploadLogs() {
        // 上传逻辑
    }

    bool hasPendingLogs() {
        // 检查是否有待上传的日志
        return false;
    }

    std::thread thread_;
    std::atomic<bool> running_;
};

// 使用方式
void initLogUpload() {
    LogUploadThread::instance().start();
}

void cleanupLogUpload() {
    LogUploadThread::instance().stop();
}
```

---

## 三、禁止的线程使用方式

### ❌ 错误示例 1：为每次操作创建线程

```cpp
// ❌ 错误：为每次点歌创建线程
void playSong(const std::string& url) {
    std::thread([url]() {
        // 播放逻辑
        TPlayerStart(url.c_str());
    }).detach();  // 错误！线程创建/销毁成本高，可能导致内存泄漏
}

// ✅ 正确：使用常驻 Player Worker
void playSong(const std::string& url) {
    PlayerCmd cmd;
    cmd.type = PlayerCmdType::PLAY;
    cmd.url = url;
    PlayerWorker::instance().enqueue(cmd);  // 正确！
}
```

### ❌ 错误示例 2：busy-loop（空转）

```cpp
// ❌ 错误：busy-loop，消耗 CPU
void workerLoop() {
    while (running_) {
        if (!queue_.empty()) {
            processTask();
        }
        usleep(1000);  // 错误！即使没有任务也在空转
    }
}

// ✅ 正确：阻塞等待
void workerLoop() {
    while (running_) {
        std::unique_lock<std::mutex> lock(mutex_);
        condition_.wait(lock, [this]() {
            return !queue_.empty() || !running_;
        });  // 正确！无任务时阻塞，不消耗 CPU
        
        if (!running_) break;
        
        Task task = queue_.front();
        queue_.pop();
        lock.unlock();
        
        processTask(task);
    }
}
```

### ❌ 错误示例 3：UI 回调中创建线程

```cpp
// ❌ 错误：UI 回调中创建线程
void onPlayButtonClicked(lv_event_t* e) {
    std::thread([this]() {
        PlayerService::instance().play(current_song_url);
    }).detach();  // 错误！线程创建成本高，可能导致资源泄漏
}

// ✅ 正确：发送命令到队列
void onPlayButtonClicked(lv_event_t* e) {
    PlayerCmd cmd;
    cmd.type = PlayerCmdType::PLAY;
    cmd.url = current_song_url;
    PlayerCmdQueue::instance().enqueue(cmd);  // 正确！
}
```

---

## 四、线程生命周期管理

### 4.1 线程创建时机

```cpp
// App 启动时创建所有常驻线程
int app_main_init() {
    // 1. 初始化核心线程（UI 主线程已在 main 中）
    
    // 2. 启动工作线程池
    NetworkWorker::instance().start();
    PlayerWorker::instance().start();
    CacheWorker::instance().start();
    
    // 3. 启动低频守护线程
    LogUploadThread::instance().start();
    UpgradeChecker::instance().start();
    
    return 0;
}
```

### 4.2 线程退出顺序

```cpp
// App 退出时按顺序停止所有线程
void app_main_cleanup() {
    // 1. 设置全局退出标志
    g_app_running = false;
    
    // 2. 停止低频守护线程（第1）
    LogUploadThread::instance().stop();
    UpgradeChecker::instance().stop();
    
    // 3. 停止工作线程池（第2）
    CacheWorker::instance().stop();
    PlayerWorker::instance().stop();
    NetworkWorker::instance().stop();
    
    // 4. 停止 Event Loop（第3）
    EventBus::getInstance().stop();
    
    // 5. UI 主线程最后退出（第4，在 main 中处理）
}
```

### 4.3 线程安全退出检查清单

- [ ] 所有线程都有 `start()` 和 `stop()` 方法
- [ ] `stop()` 方法设置退出标志并唤醒阻塞的线程
- [ ] 工作循环检查退出标志
- [ ] 使用 `thread.join()` 等待线程退出
- [ ] 退出顺序正确（依赖关系）

---

## 📚 相关文档

- **[KTV_App线程Singleton编码规范（最终版）.md](./KTV_App线程Singleton编码规范（最终版）.md)** ⭐⭐⭐ **必读（铁律级）** - 完整的模板代码、避坑指南和工程铁律（推荐优先阅读）
- [线程架构基线（最终版）.md](../线程架构基线（最终版）.md)
- [KTV_App稳定性与自愈设计说明.md](../sdk/KTV_App稳定性与自愈设计说明.md)
- [消息队列完整指南.md](../消息队列完整指南.md)

---

**最后更新**: 2025-12-30  
**维护者**: 项目团队  
**状态**: 已合并到编码规范，保留作为历史参考

