...
> **架构设计**：详见 [C++架构设计-预分配内存版本.md](./C++架构设计-预分配内存版本.md) ⭐ **主要实现文档**  
> **编码规范**：详见 [C++编码规范与避坑指南.md](./C++编码规范与避坑指南.md)

> ⚠️ **重要提示**：本文档中的 moodycamel 相关内容为**未来优化选项**，MVP阶段统一使用 `std::queue + std::mutex`（标准库）。详见 [消息队列实现与最佳实践.md](../消息队列实现与最佳实践.md)
...
#### 方案1：moodycamel::ConcurrentQueue ⚠️ **未来优化选项（MVP阶段不使用）**

> **注意**：MVP阶段统一使用 `std::queue + std::mutex`（标准库），不使用 moodycamel。  
> 如果未来性能不够，可以升级到 moodycamel::ConcurrentQueue。详见 [消息队列实现与最佳实践.md](../消息队列实现与最佳实践.md)
...
**选择理由：**
- ✅ **GitHub Stars最多**：10,000+ stars，社区认可度高
- ✅ **更新活跃**：2024年有更新，项目维护良好
- ✅ **功能全面**：支持SPSC和MPSC，未来扩展性好

#### 方案2：moodycamel::ReaderWriterQueue ⚠️ **未来优化选项（MVP阶段不使用）**

> **注意**：MVP阶段统一使用 `std::queue + std::mutex`（标准库），不使用 moodycamel。  
> 如果未来性能不够，可以升级到 moodycamel::ReaderWriterQueue。详见 [消息队列实现与最佳实践.md](../消息队列实现与最佳实践.md)
...
#### 方案3：tinySPSCQueue（如果只需要SPSC）⚠️ **需验证**
...
- ✅ 预分配内存：支持固定大小缓冲区
- ⚠️ **需验证**：
  - GitHub Stars数量（建议>1000）
  - 更新频率（建议最近6个月内有更新）
  - GitHub仓库、许可证、API、稳定性、文档

**选择建议：**
- 如果tinySPSCQueue的star数>1000且最近有更新，可以考虑
- 否则优先选择moodycamel::ReaderWriterQueue或ConcurrentQueue

**详见**：[线程与事件系统设计.md](./线程与事件系统设计.md)
...
**预分配内存适配：**
```cpp
// plog支持自定义Appender，可以使用固定大小缓冲区
class FixedBufferAppender : public plog::IAppender {
    char buffer_[4096];  // 预分配缓冲区
    // ... 实现
};
```

#### 方案2：简单日志宏（最轻量）⭐ **推荐（如果追求极简）**
...
// 使用
LOG_INFO("Application started");
LOG_ERROR("Failed to load config: %s", error_msg);
```
...
- **特点**：
  - 功能强大
  - 但可能较重，不适合嵌入式
- **GitHub Stars**：⭐ 20,000+（极高）
- **更新频率**：✅ 活跃维护
- **推荐度**：⭐⭐（不推荐，太重，不适合嵌入式）

**选择建议：**
- **如果需要功能完整的日志库**：推荐 **plog**（2,000+ stars，轻量级，适合嵌入式）
- **如果追求极简**：推荐 **简单日志宏**（无外部依赖，最轻量）
- **不推荐**：spdlog（虽然star数多，但太重，不适合嵌入式）
...
├── cJSON（JSON解析）⭐ 必需
├── std::queue + std::mutex（消息队列，标准库）⭐ 必需（MVP阶段）
├── moodycamel::ConcurrentQueue（消息队列）⚠️ 未来优化选项（MVP阶段不使用）
├── inih（配置文件解析，可选）
├── plog（日志库，可选，推荐）
└── 标准库（C/C++标准库）
    ├── 字符串处理
...
- HTTP客户端 → 使用 **libcurl**
- 配置文件解析器 → 使用 **inih**
- 消息队列 → MVP阶段使用 **std::queue + std::mutex**（标准库）  
  未来优化选项：**tinySPSCQueue**（SPSC，需验证）或 **moodycamel::ReaderWriterQueue**（SPSC）或 **moodycamel::ConcurrentQueue**（MPSC）
- 复杂的数据结构 → 使用标准库容器
- 日志系统 → 使用 **plog**（推荐）或简单宏（极简）

**✅ 使用现成开源库：**
- **cJSON** - JSON解析 ⭐ 必需
- **libcurl** - HTTP REST API客户端 ⭐ 必需
- **消息队列库** ⭐ 必需（MVP阶段使用 `std::queue + std::mutex`，未来优化选项：tinySPSCQueue、moodycamel::ReaderWriterQueue或moodycamel::ConcurrentQueue）
- **inih** - 配置文件解析（可选）
- **plog** - 日志库（可选，推荐）
- **标准库** - 数据结构、字符串、文件操作
...
| moodycamel::ConcurrentQueue | 消息队列 | 单头文件 | ⭐⭐⭐⭐⭐ **必需** |
| C/C++标准库 | 基础功能 | 系统库 | ⭐⭐⭐⭐⭐ |
...
5. **消息队列库** ⭐ 必需（线程间通信，解耦合）
   - **MVP阶段**：`std::queue + std::mutex`（标准库）⭐ 当前使用
   - **未来优化选项**：
     - **tinySPSCQueue**（如果只需要SPSC，需验证）
     - **moodycamel::ReaderWriterQueue**（如果只需要SPSC）
     - **moodycamel::ConcurrentQueue**（如果需要MPSC）
6. C/C++标准库 - 基础功能

**可选：**
7. inih - 配置文件解析
8. plog - 日志库（推荐，2,000+ stars）
...
**记住**：严格遵循避免造轮子原则。**优先选择GitHub Stars多、更新频次近的开源库**（建议star数>1000，最近6个月内有更新）。服务器端协议已确定（HTTP REST API + JSON），客户端使用libcurl（40,000+ stars） + cJSON（10,000+ stars）实现即可。消息队列优先选择moodycamel::ConcurrentQueue（10,000+ stars，2024年更新）。所有功能优先使用现成的开源库，避免自己实现。通过使用现成的开源库，可以大幅减少代码量，提高开发效率，降低维护成本。优先选择轻量级、适合嵌入式的库。
