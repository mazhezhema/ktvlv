...
- ✅ **避免造轮子**：严格遵循避免造轮子原则，优先使用开源库（libcurl、cJSON、inih等），低代码实现
...
- ✅ **必须使用 JsonHelper 封装**：业务层禁止直接使用 cJSON API
- ✅ **cJSON 是工具，不是模型**：解析完就丢，不在系统里长期持有 cJSON 对象
- ✅ **线程隔离**：JSON 解析必须在 Worker 线程，禁止在 UI 线程解析
- ✅ **大小限制**：必须有上限保护（MAX_JSON_SIZE = 64KB）
- ✅ **失败即丢**：解析失败立即返回，不做部分成功
- ✅ **详见**：[JSON解析编码规范.md](./JSON解析编码规范.md)

### SQLite 使用规范

- ✅ **必须使用 SqliteHelper 封装**：业务层禁止直接使用 sqlite3 API
- ✅ **进程唯一 DB**：SqliteHelper 是进程级 Singleton，全项目只有一个数据库连接
- ✅ **Init 只做 open**：SqliteHelper::Init() 不负责建表
- ✅ **建表在业务初始化**：建表失败 = 启动失败，直接报错退出
- ✅ **Query 返回字符串**：所有查询结果都是字符串，由业务层解释类型
- ✅ **禁止 UI 层调用**：SqliteHelper 只在 Service 层使用
- ✅ **详见**：[Utils工具类使用指南.md](./Utils工具类使用指南.md)
