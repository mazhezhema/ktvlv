...
void NetworkWorker::start() {
    EventBus::getInstance().subscribe(...);  // 正确！在 start() 中初始化
}
```
...
static NetworkWorker& g_worker = NetworkWorker::instance();  // 错误！

// ✅ 正确：App 主退出时显式 stop()
void app_main_cleanup() {
    NetworkWorker::instance().stop();      // 正确！显式停止
...
### 为什么在 Tina/F133 项目中使用 Singleton 是合理的？

1. **资源唯一性天然成立**
   - SQLite、TPlayer、libcurl、LVGL 都是全局唯一资源
   - 线程 ≈ 资源代理，天然需要全局唯一
...
**详细说明**：详见 [线程Singleton白名单与实现规范.md](./guides/线程Singleton白名单与实现规范.md)
