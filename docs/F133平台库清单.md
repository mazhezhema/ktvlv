...
## ✅ F133 平台必需库（5个必需 + 2个推荐/可选）

> **注意**：MVP阶段统一使用 `std::queue + std::mutex`，不使用 moodycamel::ConcurrentQueue
...
- **注意**: MVP阶段统一使用 `std::queue + std::mutex`，不使用 moodycamel::ConcurrentQueue
- **未来优化**: 
  - ⚠️ 如果UiEventQueue出现性能瓶颈，可以升级到 `moodycamel::ConcurrentQueue` 或 RingBuffer
  - ✅ 但当前MVP阶段不需要
...
| 集成方式 | 库数量 | 库列表 |
|---------|--------|--------|
| **FetchContent** | 6 | LVGL, lv_drivers, plog, inih, cjson, libwebsockets |
| **系统包管理器** | 1 | libcurl |
...
**原因**:
1. ✅ **核心库通用**: LVGL、libcurl、cJSON、plog、inih 都是跨平台的，F133 上同样可用
...
if(KTV_PLATFORM_F133_LINUX)
    ...
    # 通用库（FetchContent）
    FetchContent_MakeAvailable(lvgl cjson plog inih libwebsockets)
