...
- [ ] 是否在高频循环中打印 ERROR/WARN 日志？
  - ❌ **发现即驳回（硬规则）**，ERROR/WARN 日志禁止在高频循环中打印

- [ ] 是否跨线程直接更新 UI？
  - ❌ **发现即驳回（硬规则）**，UI 只能在 UI 线程更新，其他线程只能通过 Event/Message 通知

- [ ] 是否在回调中做阻塞操作？
  - ❌ **发现即驳回（硬规则）**，禁止在回调中做阻塞操作

- [ ] 是否在 UI 线程访问网络/IO？
  - ❌ **发现即驳回（硬规则）**，禁止在 UI 线程访问网络/IO

- [ ] 是否在析构函数中阻塞等待外部资源？
  - ❌ **发现即驳回（硬规则）**，禁止在析构函数中阻塞等待外部资源

- [ ] 是否模块间相互持有裸指针？
  - ❌ **发现即驳回（硬规则）**，禁止模块间相互持有裸指针

- [ ] 是否隐式启动后台线程？
  - ❌ **发现即驳回（硬规则）**，禁止隐式启动后台线程

- [ ] 是否用 sleep 作为同步手段？
  - ❌ **发现即驳回（硬规则）**，禁止用 sleep 作为同步手段

- [ ] 是否直接调用第三方 API（TPlayer、libcurl、cJSON）？
  - ❌ **发现即驳回（硬规则）**，三方库必须通过 Adapter 层访问

- [ ] 是否使用 wall clock 参与业务判断？
  - ❌ **发现即驳回（硬规则）**，所有业务时间统一使用 monotonic clock
